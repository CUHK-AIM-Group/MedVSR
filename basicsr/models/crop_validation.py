...
def forward_crop(x, model, lq_size=64, scale=4, overlap=32, flow_opt=False, dim=4):
        ...
        for t in range(T):
            for i in range(I):
                ...
                for j in range(J):
                    ...
                    B_start = B * (i * J + j)
                    B_end = B_start + B
                    # print(x.shape, x_crops.shape)
                    # print(t, i_start, i_end, j_start, j_end)
                    # print(B_start, B_end, t, pi_start, pi_end, pj_start, pj_end)
