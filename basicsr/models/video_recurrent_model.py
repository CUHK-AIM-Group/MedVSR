...
class VideoRecurrentModel(VideoBaseModel):
    ...
    def optimize_parameters(self, current_iter, teacher_outputs=None):
        ...
        super(VideoRecurrentModel, self).optimize_parameters(current_iter, teacher_outputs)
                ...
                for idx in range(visuals['result'].size(1)):
                    ...
                    if with_metrics:
                        ...
                        for metric_idx, opt_ in enumerate(self.opt['val']['metrics'].values()):
                            ...
                            #         f.write('\n')
                            #######################################################################
                            # print("At folder: ", folder, " idx: ", idx, " metric_idx: ", metric_idx, " result: ", result)
    ...
    def test(self):
        # print("===>, using test function in VideoRecurrentModel")
        n = self.lq.size(1)
        self.net_g.eval()
        ...
        with torch.no_grad():
            if "crop" in self.opt and self.opt['crop']:
                ...
                self.output = forward_crop(self.lq, self.net_g, lq_size=lq_size, overlap=overlap, dim=5)
                print("===>, using crop validation in VideoRecurrentModel, Output shape: ", self.output.shape)
            else:
