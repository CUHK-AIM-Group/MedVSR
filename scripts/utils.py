...
def recursively_imread(img_path, flags):
    ...
    while retry > 0:
        try:
            ...
            logging.warning('Read image error: %s, remaining retry times: %d', e, retry - 1)
            ...
            retry -= 1
    if img is None:
        logging.error('There exists problem with %s', img_path)
    return img
...
def recursively_imwrite(saved_img_path, img):
    ...
    while retry > 0:
        try:
            ...
            logging.warning('Write image error: %s, remaining retry times: %d', e, retry - 1)
            time.sleep(1)  # sleep 1s
        else:
            break
        finally:
    ...
    if write_flag is False:
        logging.error('There exists problem with %s', saved_img_path)
    return write_flag
